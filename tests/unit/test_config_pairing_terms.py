...
that differed in no rule at all. They moved into private config and bind as
terms settled in the thread.

**Which side of the contract they arrive from is the opponent's to decide.**
imreeyal's 9-key shape omitted them; the najamjad contract we later agreed
carries them outright. So "absent from the contract" is a fact about one
agreement, not an invariant, and asserting it here failed the moment the active
contract changed. What survives every agreement is the half that actually bites:
...
* **Forgetting we still need the values.** They are not decoration: three are
  ...
  — `pheromones.decay_model` to `"multiplicative"`, the model we did not agree,
  silently reblurring the belief filter; the two seal flags to `False`.
"""
