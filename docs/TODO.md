...
(expectimax, depth 2), `core/domain/trail.py` (our own emission, reconstructed), `thief/anchor.py` (the
...
- [x] 10.2.2 [D] - Visualisations | DoD: High resolution, labelled axes, consistent accessible palette. (X §9.3) — **Done 24/08 — every sub-item below closed; all figures regenerate from `notebooks/results_analysis.ipynb`, which executes end to end with 0 errors and 0 ruff violations.**
  - [x] 10.2.2.a [D] - Belief heatmap snapshots | DoD: Shows convergence onto the opponent. — **Done 24/08 — `results/fig_belief_convergence.png`, notebook §4. The shipped `predict`/`update_from_scent`, not a proxy. Two inputs side by side: a single fresh deposit puts the peak on the true cell 36/36 steps; the transmitted trail a match really sends puts it there 2/36 and entropy *rises* 0.35→1.58 bits. The second is the honest one and it corroborates §6.**
  ...
  - [x] 10.2.2.c [D] - Win-rate bars by opponent and role | DoD: Both roles represented. — **Done 24/08 — `results/fig_winrate_by_opponent_role.png`, notebook §6. Both roles represented, from the filed counted series de-duplicated on `game_uid`: Cop 0/6, Thief 2/6.**
  - [x] 10.2.2.d [I] - Reliability coefficient over time | DoD: Demonstrates we detected opponent deception. *(Showpiece.)* — **Done 24/08 — `results/fig_reliability_over_time.png`, notebook §7. (a) the shipped `Reliability` class catches a scripted liar: coefficient 0.66→0.92→0.41, decayed γ=0.85 crosses 0.5 within three turns. (b) the honest empirical half: imreeyal spoke 193 hints and **0** were checkable (invented landmarks), vibecode sent none — the detector was never exercised in a real match.**
...
- [x] 10.2.3 [D] - Belief-convergence animated GIF | DoD: Embedded in the README. *(P3)* — **Done 24/08 — `results/belief_convergence.gif`, 36 frames, embedded in both READMEs.**
...
- [x] 10.3.2 [D] - Edge-case catalogue with screenshots | DoD: Opponent disconnect · malformed hint · tunnel drop mid-protocol · LLM timeout · hash mismatch. Each ends in a clean technical loss with a persisted log. (X §6.3, Ch. 11) — **Done 24/08.** Disconnect (`edge-case-disconnect-*.png`), tunnel drop (`reply-najamjad-warmup1-findings.md`, from a real opponent), hash mismatch (`edge-case-hash-mismatch.txt` + the rewritten log: a *legal* move swapped after the fact turns `Verified OK`/exit 0 into `FAILED`/exit 1) and malformed hint (`edge-case-malformed-hint.txt`, 11 hostile inputs, none raised). **LLM timeout is N/A and documented as such** in RESEARCH-REPORT §5, on the §4 principle: `tokens_total_series` is `0` for both sides in every filed series and the only `model` any peer put on the wire is `"none"`, so no match ever had a provider in the loop and a provider timeout cannot reach the game loop.
...
- [x] 10.3.5 [D] - Finalise `docs/CONTRADICTIONS.md` | DoD: Each contradiction, our choice and our reasoning. — **Done 24/08 — audited for internal consistency: C-016, C-017 and C-019 were in the body but missing from the index; C-003/C-011a/C-012 verified as properly retired in the appendix. Index and body now agree exactly.**
- [x] 10.3.6 [D] - Finalise `docs/PROMPT_LOG.md` | DoD: Prompts, context, outputs, iterations, lessons. (X §8.3) — **Done 24/08 — coverage gap between 28/07 and 24/08 stated rather than back-filled, three new entries from real episodes (the mirror-delete near-miss, the flattering-measurement inversion, the 150-LOC gate forcing an extraction), lessons 6–8 added.**
...
- [x] 10.4.5 [D] - §5 Screenshots — **absolute requirement** | DoD: Belief heatmap from the Live GUI **and** `Verified OK` from the Replay App. (Ch. 9) — **Done 24/08 — both required screenshots embedded: `m7-live-gui-belief.png` and `m7-replay-verified-ok.png`, in RESEARCH-REPORT §5 and in a new Screenshots section of both READMEs. They existed on disk but were only named, never shown.**
...
- [x] 10.QG.3 [D] - `uv run pytest --cov` | DoD: All pass; coverage ≥85 %. — **Verified 24/08 — 1846 passed, 13 deselected, coverage 92.17 % against the 85 % gate.**
